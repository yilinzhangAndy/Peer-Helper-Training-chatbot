...
def load_components():
    ...
    try:
        ...
        # Load cost controller
        cost_controller = CostController()
        
        # Create cost-aware pipeline
        cost_aware_pipeline = CostAwarePipeline(pipeline, cost_controller)
        
        return intent_classifier, spm, cost_aware_pipeline, cost_controller
...
def main():
    ...
    with st.spinner("Loading AI components..."):
        intent_classifier, spm, pipeline, cost_controller = load_components()
    
    if not all([intent_classifier, spm, pipeline, cost_controller]):
        st.error("Failed to load system components. Please check your setup.")
        return
    
    # Get user ID and check usage
    user_id = get_user_id()
    remaining, total = cost_controller.get_usage_info(user_id)
    
    # Show cost info in sidebar
    show_cost_info()
    show_usage_warning(remaining, total)
    ...
    with st.sidebar:
        ...
        if selected_persona:
            ...
            st.markdown(f"""
            <div class="persona-card persona-selected">
